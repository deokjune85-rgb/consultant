# =====================================================
# 💰 IMD BIZ-FINDER FINAL (Document UI Fixed)
# Authorized by: The Architect
# =====================================================
import streamlit as st
import streamlit.components.v1 as components  # ★ 핵심: 강제 HTML 렌더링용
import pandas as pd
import numpy as np
...
if st.session_state.analysis_done:
    ...
    if st.session_state.show_spinner:
        ...
        for i, log in enumerate(logs):
            time.sleep(random.uniform(0.1, 0.3)) # 랜덤 딜레이
        ...
        bar.empty()
        st.session_state.show_spinner = False # 스피너 비활성화
    ...
    # [F. ★★★ 가짜 서류 시뮬레이션 (Iframe 강제 렌더링) ★★★]
    # 스트림릿의 다크모드 간섭을 피하기 위해 Iframe으로 격리시킴
    if st.session_state.psst_generated:
        st.markdown("---")
        st.markdown("### 🖨️ 생성된 사업계획서 (미리보기)")
        
        # HTML + CSS (A4 용지 스타일 강제 적용)
        report_html = f"""
        <!DOCTYPE html>
        <html>
        <head>
        <style>
            ...
            .paywall-text {{ color: white; font-size: 16px; }}
            .paywall-sub {{ color: #ccc; font-size: 12px; font-weight: normal; }}
        </style>
        </head>
        <body>
        ...
        <div class="a4-paper">
            ...
            <table>
                <tr>
                    <th style="width: 15%;">업체명</th> <td style="width: 35%;">(주)IMD솔루션</td> 
                    <th style="width: 15%;">대표자</th> <td style="width: 35%;">김준</td>
                </tr>
                <tr>
                    <th>설립일</th> <td>{2025-biz_year}.01.01</td> 
                    <th>업종</th> <td>{biz_type}</td>
                </tr>
                <tr>
                    <th>매출액</th> <td>{sales}억 원</td> 
                    <th>상시근로자</th> <td>{employee_count}명</td>
                </tr>
            </table>
            ...
            <table>
                <tr>
                    <th style="width: 20%;">소요 자금</th> 
                    <td style="width: 30%;">운전자금 300,000,000원</td> 
                    <th style="width: 20%;">자금 용도</th> 
                    <td style="width: 30%;">원부자재 구입 및 R&D 인건비</td>
                </tr>
            </table>

            <p><strong>3. 사업 내용 및 기대 효과 (PSST 핵심)</strong></p>
            <div style="border: 1px solid #ddd; padding: 15px; background-color: #fafafa;">
            ...
            <div class="paywall-overlay">
                <div class="paywall-text">🔒 PREMIUM REPORT</div>
                <div class="paywall-sub">(유료 버전에서 전체 다운로드 가능)</div>
            </div>
        </div>
        </body>
        </html>
        ...
        # Iframe으로 격리하여 렌더링 (높이 850px, 스크롤 가능)
